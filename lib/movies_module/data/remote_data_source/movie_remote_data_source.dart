...
import 'package:movies_app/movies_module/data/model/movie_Details_model.dart';
import 'package:movies_app/movies_module/data/model/movie_model.dart';
import 'package:movies_app/movies_module/data/model/recommendation.dart';
...
class MovieRemoteDataSource extends BaseMovieRemoteDataSource {
  ...
  Future<List<MovieModel>> getNowPlayingMovies() async {
    final response = await Dio().get(AppConstant.nowPlaying);
  ...
  Future<List<MovieModel>> getPopularMovies() async {
    final response = await Dio().get(AppConstant.popular);
    if (response.statusCode == 200) {
      return List<MovieModel>.from((response.data["results"] as List)
          .map((e) => MovieModel.fromJson(e)));
    } else {
  ...
  Future<List<MovieModel>> getTopRatedMovies() async {
    final response = await Dio().get(AppConstant.topRated);
    if (response.statusCode == 200) {
      return List<MovieModel>.from((response.data["results"] as List)
          .map((e) => MovieModel.fromJson(e)));
    } else {
  ...
  Future<List<RecommendationModel>> getRecommendation(RecommendationParameters parameters) async{
    final response = await Dio().get(AppConstant.recommendation(parameters.id));
    if (response.statusCode == 200) {
